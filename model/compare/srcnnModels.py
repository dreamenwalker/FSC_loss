...
class SRCNN(nn.Module):
    ...
    def __init__(self, mode: str = "train", init_weights: bool = True):
...
def _srcnn(arch: str, pretrained: bool, progress: bool, **kwargs: Any):#
...
def srcnn_x2(pretrained: bool = False, progress: bool = True, **kwargs: Any):
    r""" `"Image Super-Resolution Using Deep Convolutional Networks" <https://arxiv.org/pdf/1501.00092v3.pdf>`_.
...
class SRCNN2channels(nn.Module):
    r""" Construct SRCNN super-resolution model.
    ...
    def __init__(self, mode: str = "train", init_weights: bool = True) -> None:
        super(SRCNN2channels, self).__init__()
        # The model does not need to fill the edges during the training process, 
        # and needs to fill the edges during the testing mode.
        if mode == "train":
            padding = False
        ...
        # Feature extraction layer.
        self.features = nn.Sequential(
            nn.Conv2d(2, 64, 9, 1, 0 if not padding else 4),
            nn.ReLU(True)
        )
        ...
        # Reconstruction the layer.
        self.reconstruction = nn.Conv2d(32, 2, 5, 1, 0 if not padding else 2)
